...
public class FlickToDismissViewController: UIViewController {
    ...
    private func setup() {
        // Setup pan gesture
        panGestureRecognizer = UIPanGestureRecognizer(target: self, action: #selector(FlickToDismissViewController.handleAttachmentGesture(panGesture:)))
        // Setup animator
        ...
        var imageView = UIImageView(frame: CGRect(x: 20, y: 0, width: flickableView.frame.size.width, height: flickableView.frame.size.height))
        imageView.image = UIImage(named: imageName);
        flickableView.addSubview(imageView)
    }
